...
namespace Gem {
	...
	void CollisionWorld::step() const {
		...
		for (Collider::Collision& collision : collisions) {
			/*Gem::Speed speed = dot(collision.normal, movingObject.velocity - staticObject.velocity);
			if (speed > 0) {
				Gem::KilogramMeterPerSeconds impulse = 2 * speed / (movingObject.mass + staticObject.mass);
