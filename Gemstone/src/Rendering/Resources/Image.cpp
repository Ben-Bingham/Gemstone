...
namespace Gem {
	Image::Image(std::string path, bool flipVertically)
		...
		if (!data) {
			LOG("Image: " + m_Path + " Failed to load. ", LogLevel::ERROR);
		}
	...
	Image::Image(const Vector3f& colour, const unsigned int width, const unsigned int height)
		: m_Width(width)
		, m_Height(height)
		, m_Channels(3) {
	...
	Image::Image(const Vector4f& colour, const unsigned int width, const unsigned int height)
		: m_Width(width)
		, m_Height(height)
		, m_Channels(4) {
