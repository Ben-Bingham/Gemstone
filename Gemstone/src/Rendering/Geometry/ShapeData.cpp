...
namespace Gem {
	...
	Vertices ShapeData::getSphereVertices(const unsigned horizontalResolution, const unsigned verticalResolution) {
		...
		const Radian sectorStep = 2 * pi / (float)horizontalResolution;
		const Radian stackStep = pi / (float)verticalResolution;

		for (unsigned int i = 0; i <= verticalResolution; i++) {
			const Radian stackAngle = pi / 2 - i * stackStep; // goes from top to bottom

			for (unsigned int j = 0; j <= horizontalResolution; j++) {
				const Radian sectorAngle = j * sectorStep;
