...
namespace Gem {
	DebugRenderer::DebugRenderer(RenderingSystem* renderer) 
		: m_Mesh(CreatePtr<MeshData>())
...
#ifdef RUBY_DEBUG
		if (points.size() % 3 == 1) {
			LOG("Uneven number of values supplied, this may modify futre queue calls.", LogLevel::WARNING);
		}
#endif
	...
	void DebugRenderer::queue(const std::vector<Vector3f>& points) {
#ifdef RUBY_DEBUG
		if (points.size() % 2 == 1) {
			LOG("Uneven number of points supplied, this may modify futre queue calls.", LogLevel::WARNING);
		}
#endif
	...
	void DebugRenderer::queue(const Ptr<MeshData>& mesh, const Vector3f& position, const Vector3f& scale) {
		const Vertices vertices = mesh->getVertices();

		if (vertices.empty()) {
			LOG("Empty mesh supplied, noting will be queued.", LogLevel::WARNING);
			return;
		}

		Matrix4f transformMatrix{ 1.0f };
		transformMatrix.translate(position).scale(scale);

		std::vector<Vector3f> points;
		for (const Vertex& vertex : vertices) {
			const Vector3f vector = Vector3f{ Vector4f{vertex.position, 1.0f} *transformMatrix };
