...
namespace Gem {
	...
	public:
...
#ifdef RUBY_DEBUG
			if (data.empty()) {
				LOG("Attempting to initilize a buffer with no data. Consider using setNoData() instead.", LogLevel::WARNING);
			}

			if (m_Initialized) {
				LOG("Attempting to place data into preinitilized buffer. Please use setSubData() instead.", LogLevel::WARNING);
			}
#endif
		...
		void setData(const std::vector<Vector2<T>>& data, const int usage = GL_STATIC_DRAW) {
			bind();
		...
		void setData(const std::vector<Vector3<T>>& data, const int usage = GL_STATIC_DRAW) {
			bind();
		...
		void setData(const std::vector<Vector4<T>>& data, const int usage = GL_STATIC_DRAW) {
			bind();
...
#ifdef RUBY_DEBUG
			if (!m_Initialized) {
				LOG("Attempting to place data into a buffer that has not been initilized yet. Please use setData() instead.", LogLevel::WARNING);
			}
#endif
			const size_t dataSize = data.size() * sizeof(T);
#ifdef RUBY_DEBUG
			if (dataSize > m_Size || dataSize + offset > m_Size) {
				LOG("Attempted to modify data outside of buffer range, operation not possible.", LogLevel::ERROR);
			}
#endif
			bind();
		...
		void setSubData(const std::vector<Vector2<T>>& data, const unsigned int offset = 0) {
#ifdef RUBY_DEBUG
			if (!m_Initialized) {
				LOG("Attempting to place data into a buffer that has not been initilized yet. Please use setData() instead.", LogLevel::WARNING);
			}
		...
		void setSubData(const std::vector<Vector3<T>>& data, const unsigned int offset = 0) {
#ifdef RUBY_DEBUG
			if (!m_Initialized) {
				LOG("Attempting to place data into a buffer that has not been initilized yet. Please use setData() instead.", LogLevel::WARNING);
			}
		...
		void setSubData(const std::vector<Vector4<T>>& data, const unsigned int offset = 0) {
#ifdef RUBY_DEBUG
			if (!m_Initialized) {
				LOG("Attempting to place data into a buffer that has not been initilized yet. Please use setData() instead.", LogLevel::WARNING);
			}
	...
	template<typename T>
	using BlankBuffer = Gem::GlBuffer<T, GL_TEXTURE_BUFFER>;
